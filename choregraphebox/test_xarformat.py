...
class TestXARformat(unittest.TestCase):
    ...
    def test_load_box(self):
        ...
        self.assertFalse(box.has_plugin)
        ...
        self.assertEquals(items[2].name, "Test3")
        box = items[2].get_box()
        self.assertEquals(box.name, "Test3")
        self.assertTrue(box.has_plugin)
        ports = box.get_inputs()
        ...
        self.assertEquals(items[3].name, "Test4")
        box = items[3].get_box()
        self.assertEquals(box.name, "Test4")
        self.assertFalse(box.has_plugin)
        ports = box.get_inputs()
        self.assertEquals(len(ports), 2)
        self.assertEquals(ports[0].name, "onLoad")
        self.assertEquals(ports[1].name, "Set Language_onSet")
        ports = box.get_outputs()
        self.assertEquals(len(ports), 1)
        self.assertEquals(ports[0].name, "Log_onDone")
        params = box.get_parameters()
        self.assertEquals(len(params), 0)
